...
bool available(bitset<4> spot) // helper function to check if a location is available
{
...
void build_graph(void)
{
    ...
    while (curr_state != end) 
    {
        for (int i = 0; i < obj; i++) 
        {
            if (curr_state[me] == curr_state[i]) 
            {

                bitset<obj> nextState = curr_state;

                nextState[me].flip();

                i != me ? nextState[i].flip() : false;

                if (available(nextState)) // check next state by calling helper
                {

                    int curr = (int)curr_state.to_ulong();
        ...
        curr_state = (int)curr_state.to_ulong() + 1;
    }
}
