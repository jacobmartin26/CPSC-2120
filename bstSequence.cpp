...
Node* insert(Node* T, int v, int r)
{
    ...
    int curr = T->left ? T->left->size : 0; // keeps track of current rank

    if (curr >= r)
    {
        T->left  = insert(T->left, v, r); // recursive call for left
    }
    else
    {
        T->right = insert(T->right, v, r - curr - 1); // recursive for right
    }

    fix_size(T); // maintain correct size
...
void split(Node* T, int r, Node** L, Node** R)
{
    ...
    if (T == nullptr) 
    {
        *L = *R = nullptr; // empty tree

        return;
    }


    int myRank = T->left ? T->left->size : 0; // another variable to keep track of rank

    
    if (myRank < r) // node on left side
    {
        split(T->right, r - myRank - 1, &T->right, R); // recursive call

        *L = T;
    }

    
    else // node on right side
    {
        split(T->left, r, L, &T->left); // recursive call

        *R = T;
    }

    fix_size(T); // maintain size
}
...
Node* insert_random(Node* T, int v, int r)
{
    ...
    bool chance  = (rand() % (T->size + 1)) == 0; // chance of insertion at root

    int currRank = T->left ? T->left->size : 0; // another variable to hold rank
    ...
    if (chance) // chance true
    {
        ...
        L = R = nullptr; // empty nodes

        split(T, r, &L, &R); // split tree

        root->left  = L; // set val of root on left and right
        root->right = R;

        fix_size(root); // maintain size

        return root;
    }

    if (r > currRank) // randomly insertion of k
    {
        T->right = insert_random(T->right, v, r - currRank - 1); // recursive call for right
    }

    else
    {
        T->left  = insert_random(T->left, v, r); // for left
    }
